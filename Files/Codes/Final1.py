...
# Ensemble ağırlıkları: ortalama tüketim modeline daha fazla önem
W_ORT  = 0.60  
W_FULL = 0.40 
...
def show_input_plots():
    ...
    plt.title("Aylık Tüketim"); plt.xlabel("Ölçüm"); plt.ylabel("Tüketim (kWh)"); plt.tight_layout(); plt.show()

    plt.figure(); plt.plot(range(1,len(d)+1),d,marker='o')
    plt.title("Demand"); plt.xlabel("Ölçüm"); plt.ylabel("Demand (kW)"); plt.tight_layout(); plt.show()
...
def show_main_plots():
    ...
    plt.figure(); plt.imshow(cm,cmap=plt.cm.Blues); plt.title("CM")
...
def prompt_and_show_user_plot():
    ...
    def plot_sel():
        ...
        plt.title("Aylık Tüketim Karşılaştırma"); plt.xlabel("Ay"); plt.ylabel("Tüketim (kWh)")
        plt.legend(); plt.tight_layout(); plt.show()
...
def show_result_window(p):
    ...
    btn=tk.Button(win,text="Kapat",command=win.destroy,font=FONT_NORMAL,
                  ...
                  bd=0,relief="flat",width=BUTTON_WIDTH,height=BUTTON_HEIGHT,
                  padx=BUTTON_PADX,pady=BUTTON_PADY)
    btn.pack(pady=10); btn.bind("<Enter>",on_enter); btn.bind("<Leave>",on_leave)
...
ort_entry=add_labeled("Aylık Tüketim (kWh) (virgülle):")
dem_entry=add_labeled("Demand (kW) (virgülle):")
...
def clear_all():
    ort_entry.delete("1.0",tk.END)
    dem_entry.delete("1.0",tk.END)

btn_clear=tk.Button(action_row,text="❌ Temizle",command=clear_all,
                    font=FONT_NORMAL,bg=BUTTON_COLOR,fg=BLACK_TEXT,
                    activebackground=BUTTON_HOVER,bd=0,relief="flat",
                    width=BUTTON_WIDTH,height=BUTTON_HEIGHT,
                    padx=BUTTON_PADX,pady=BUTTON_PADY)
